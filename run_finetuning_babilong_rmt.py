...
if __name__ == "__main__":
    ...
    try:
        noise_dataset = datasets.load_dataset(
            args.noise_dataset,
            args.noise_dataset_split,
        )
        noise_dataset_train = noise_dataset["train"]
        noise_dataset_test = noise_dataset["test"]
    except ConnectionError:
        noise_dataset_train = datasets.Dataset.from_file(
            "/home/jovyan/.cache/huggingface/datasets/pg19/default/0.1.0/64837d6fce7251337df051ca74e9a5435d1c9cb7f3033ba257826e44d338f83c/pg19-train.arrow"
        )
        noise_dataset_test = datasets.Dataset.from_file(
            "/home/jovyan/.cache/huggingface/datasets/pg19/default/0.1.0/64837d6fce7251337df051ca74e9a5435d1c9cb7f3033ba257826e44d338f83c/pg19-test.arrow"
        )
    ...
    if (
        ...
        train_sample_size = [
            int(args.segment_size * i) for i in range(1, args.max_n_segments)
        ] + [args.sample_size]
        train_sample_size = [s - qa_margin for s in train_sample_size]
        logger.info(
            f"Will be choosing sample size randomly from {train_sample_size} for training"
        )
    else:
    ...
    if (args.task_start_pct is not None) and (args.task_end_pct is not None):
        ...
        max_sentence_len = int(
            (args.task_end_pct - args.task_start_pct) * 0.5 * args.sample_size
        )
    ...
    def collate_fn(batch):
        ...
        gen_inputs = pad_sequence(
            gen_inputs, padding_value=id_pad_value, batch_first=True
        )
    ...
    if args.freeze_model_weights:
        ...
        logger.info(
            f"Remaining parameters: {[n for n, p in model.named_parameters() if p.requires_grad]}"
        )
