...
class TrainerArgs:
    model_path: Optional[str] = field(
        default=None, metadata={"help": "path where to save model (default: None)"}
    )
    log_interval: Optional[int] = field(
        default=None,
        metadata={
            "help": "log to report loss, metrics on training data every N batches (default: None)"
        },
    )
    ...
    save_interval: Optional[int] = field(
        default=None, metadata={"help": "save model every N steps (default: None)"}
    )
    save_best: bool = field(
        default=False,
        metadata={
            "help": "Save best checkpoint if validation set is provided (default: False)"
        },
    )
    use_generate_on_valid: bool = field(
        default=False,
        metadata={
            "help": "Use model.generate method when running validation step (default: False)"
        },
    )
    # load model args
    ...
    reset_lr: bool = field(
        default=False,
        metadata={
            "help": "Do not load lr_scheduler from checkpoint and setup new lr (default: False)"
        },
    )
    ...
    lr: Optional[float] = field(
        default=None, metadata={"help": "learning rate (default: None)"}
    )
    batch_size: int = field(
        default=1, metadata={"help": "input batch size for training (default: 1)"}
    )
    iters: int = field(
        default=1,
        metadata={
    ...
    clip_grad_norm: Optional[float] = field(
        default=None,
        metadata={
            "help": "torch.nn.utils.clip_grad_norm_ max_norm parameter. 0 or None is no clip (default: None)"
        },
    )
    clip_grad_value: Optional[float] = field(
        default=None,
        metadata={
            "help": "torch.nn.utils.clip_grad_value_ clip_value parameter. 0 or None is no clip (default: None)"
        },
    )
    early_stopping_patience: Optional[int] = field(
        default=None,
        metadata={
            "help": "stop training if `early_stopping_patience` subsequent evalutations did not improve value of "
    ...
    num_training_steps: Optional[int] = field(
        default=None,
        metadata={
            "help": "number of training steps for scheduler, if not set iters will be used (default: None)"
        },
    )
    ...
    optimize_metric: str = field(
        default="loss",
        metadata={
            "help": "metric name to optimize on validation set, save the best model, drop lr (default: loss)"
        },
    )
...
class Trainer:
    def __init__(
        ...
        logger.info(
            f"setting up trainer with accelerator state:\n{self.accelerator.state}"
        )
        self.model = model
        ...
        self.global_batch_size = (
            self.per_worker_batch_size * self.accelerator.num_processes
        )

        self.model_forward_args = set(
            get_fn_param_names(self.accelerator.unwrap_model(self.model).forward)
        )

        if (
            self.args.clip_grad_norm is not None
            and self.args.clip_grad_value is not None
        ):
            raise RuntimeError(
        ...
        if self.args.use_lr_drop and self.valid_dataloader is None:
            raise RuntimeError(
                "lr drop is based on validation metrics, but validation set is not set"
            )
        if self.args.use_lr_drop:
            self.lr_drop_scheduler = ReduceLROnPlateau(
                self.optimizer,
                mode=self.args.optimize_mode,
    ...
    def step(
        ...
        if not np.all(np.array(batch_sizes) == batch_sizes[0]):
            raise RuntimeError(
                f"not all elements in a batch have equal dim 0 size: {batch_sizes}"
            )
        ...
        with torch.set_grad_enabled(is_train_mode):
            for j in range(0, batch_size, self.args.batch_size):
                is_last_batch = (
                    j == (batch_size // self.args.batch_size - 1) * self.args.batch_size
                )
                grad_sync_context = (
                    contextlib.nullcontext
                    if is_last_batch
                    else self.accelerator.no_sync
                )
                    ...
                    if not is_train_mode and self.args.use_generate_on_valid:
                        ...
                        if "max_length" not in generate_kwargs and "labels" in subbatch:
                            ...
                            generate_kwargs["max_length"] = (
                                subbatch["labels"].shape[-1] + 1
                            )
                        if "attention_mask_generate" in subbatch:
                            generate_kwargs["attention_mask"] = subbatch[
                                "attention_mask_generate"
                            ].to(self.device)
                        ...
                        if "global_attention_mask" in subbatch:
                            generate_kwargs["global_attention_mask"] = subbatch[
                                "global_attention_mask"
                            ]
                        if "input_ids_generate" in subbatch:
                            generation_outputs = self.accelerator.unwrap_model(
                                self.model
                            ).generate(
                                subbatch["input_ids_generate"].to(self.device),
                                **generate_kwargs,
                            )
                        else:
                            generation_outputs = self.accelerator.unwrap_model(
                                self.model
                            ).generate(subbatch["input_ids"], **generate_kwargs)
    ...
    def _clip_gradients(self):
        ...
        if self.args.clip_grad_value:
            ...
            grad_norm = self.accelerator.clip_grad_norm_(
                params, self.args.clip_grad_norm
            )
    ...
    def _skip_n_train_batches(self, train_batches, n):
        ...
        logger.info(
            f"Skipping {n} batches from the dataset from epoch {self.n_epoch}..."
        )
        # skipping...
        for _ in tqdm(
            itertools.islice(train_batches, n),
    ...
    def _add_batch_metrics(
        self, batch_metrics: Dict[str, Union[float, torch.Tensor]], split: str
    ):
    ...
    def collect_metrics(self, split: str) -> dict:
        ...
        metrics_keys = set(
            accelerate.utils.gather_object(list(self.batch_metrics[split].keys()))
        )
        if metrics_keys != self.batch_metrics[split].keys():
            missing_metrics_keys = metrics_keys - self.batch_metrics[split].keys()
            logger.warning(
                f"some of the batch-lvl metrics on rank_{self.accelerator.process_index} are missing, "
        ...
        if self.keep_for_metrics_fn and self.metrics_fn:
            metrics_data = {}
            data_keys = set(
                accelerate.utils.gather_object(list(self.metrics_data[split].keys()))
            )
            if data_keys != self.metrics_data[split].keys():
                missing_data_keys = data_keys - self.metrics_data[split].keys()
                logger.warning(
                    f"some of the data collected from keep_for_metrics_fn on "
            ...
            for k in data_keys:
                metrics_data[k] = accelerate.utils.gather_object(
                    self.metrics_data[split][k]
                )
                ...
                if m_shape is None:
                    ...
                    metrics_data[k] = list(
                        chain.from_iterable([t.tolist() for t in metrics_data[k]])
                    )
            m = self.metrics_fn(metrics_data)
            if len(metrics.keys() & m.keys()) != 0:
                logger.warning(
                    f"metrics ({m.keys()}) and batch-lvl metrics ({metrics.keys()}) have common names. "
    ...
    def train(self) -> None:
        ...
        for batch in train_batches:
            ...
            if self.args.log_interval and self.n_iter % self.args.log_interval == 0:
                ...
                if self.accelerator.is_main_process:
                    ...
                    for k in train_metrics:
                        logger.info(
                            f"step: {self.n_iter}/{self.args.iters} {k}: {train_metrics[k]:.4f}"
                        )
                        if self.tb:
                            self.tb.add_scalar(
                                f"{k}/iterations/train", train_metrics[k], self.n_iter
                            )
                            self.tb.add_scalar(
                                f"{k}/samples/train",
                                train_metrics[k],
                    ...
                    if self.tb:
                        self.tb.add_scalar(
                            "time/iterations/per_iter", iteration_time, self.n_iter
                        )
                        self.tb.add_scalar(
                    ...
                    for j, param_group in enumerate(self.optimizer.param_groups):
                        ...
                        for p in ["lr", "scaled_lr"]:
                            if (
                                p in param_group
                                and param_group[p] is not None
                                and self.tb
                            ):
                                self.tb.add_scalar(
                                    f"{p}/iterations/param_group_{j}",
                                    param_group[p],
                                    self.n_iter,
                                )
                                self.tb.add_scalar(
                                    f"{p}/samples/param_group_{j}",
                                    param_group[p],
                                    self.n_iter * self.global_batch_size,
                                )
                    # log gradients global norm
                    gnorm = (
                        np.mean(global_grad_norms) if len(global_grad_norms) > 0 else 0
                    )
                    if self.tb:
                        self.tb.add_scalar(
                            "gradients_global_norm/iterations", gnorm, self.n_iter
                        )
                        self.tb.add_scalar(
            ...
            if (
                ...
                and self.n_iter % self.args.valid_interval == 0
            ):
                ...
                if self.metric_improved_fn(best_valid_metric, valid_metric):
                    ...
                    self.early_stopping_counter = 0
                    logger.info(
                        f"The best {self.args.optimize_metric} metric was improved to: {best_valid_metric}"
                    )
                    ...
                    logger.info(
                        f"Metric was not improved for the last #{self.early_stopping_counter} evaluations"
                    )
                if self.accelerator.is_main_process and self.tb:
                    self.tb.add_scalar(
                        "patience/iterations", self.early_stopping_counter, self.n_iter
                    )
                    self.tb.add_scalar(
                        "patience/samples",
            ...
            if (
                ...
                and self.early_stopping_counter > self.args.early_stopping_patience
            ):
        ...
        if self.accelerator.is_main_process:
            ...
            for k in metrics:
                ...
                if self.tb and write_tb:
                    self.tb.add_scalar(
                        f"{k}/iterations/{split}", metrics[k], self.n_iter
                    )
                    self.tb.add_scalar(
                        f"{k}/samples/{split}",
                        metrics[k],
    ...
    def load(
        ...
        if not reset_iteration:
            ...
            )  # as saved iteration is already performed
        ...
        if not load_only_model_ckpt:
            ...
            if reset_optimizer:
                raise RuntimeError(
                    "Reset optimizer only is not supported. You may load only model weights with"
                    "--reset_optimizer --reset_lr"
                )
            if reset_lr:
                raise RuntimeError(
                    "Reset lr & scheduler only is not supported. You may load only model weights with"
                    "--reset_optimizer --reset_lr"
                )
        else:
    ...
    def save(self, save_path, suffix="") -> None:
        if save_path is not None:
            ...
            logger.info(
                f"Model, trainer, and accelerate state were saved to {save_path}"
            )
