...
def get_dataset_df(dataset_path, max_n_facts=None):
    ...
    df["reference_num"] = df.answer.apply(
        lambda x: x if x is None else [int(n) for n in re.split("\t| ", x)[1:]]
    )
    ...
    initial_samples = [
        df[df.initial_sample_num == sn] for sn in df.initial_sample_num.unique()
    ]
        ...
        while len(sentences) == 0:
            ...
            if self.shuffle:
                if len(text) == 0:
                    continue
                text = text[
                    self.gen.choice(len(text)) :
                ]  # start from random position in text
                text = text[
                    : sample_size * 10
                ]  # cut too long texts to speed up tokenization
...
class NoiseInjectionDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        facts_tok = self.tokenizer(list(sample["facts"]))["input_ids"] # [[50, 15918, 1816, 736, 284, 262, 2607, 13], [19962, 1816, 284, 262, 12436, 13]]
        question_tok = self.tokenizer(sample["question"])["input_ids"] # [8496, 318, 7806, 30, 220]
        answer_tok = self.tokenizer(sample["answer"])["input_ids"] # [37648, 3823]

        sample_size = self.get_sample_size() # 1008
        task_len = sum_lengths(facts_tok) # 14
        background_text_len = sample_size - task_len # 1008 - 14
        background_text = self.noise_sampler.get_sample(background_text_len) # [[2437, 881, 466, 345, 892, 326, 43506, 338, 2861, 1701], [1, 42, 391, 470, 307, 2277, 338, 476, 69, ...
        sample["background_text"] = background_text

        if (
            self.task_start_pct is None and self.task_end_pct is None
        ):  # if fact position unspecified
            possible_positions = range(len(background_text) + 1) # range(0, 49)
        else:
        ...
        fact_positions = self.gen.choice(possible_positions, len(facts_tok)) # array([21, 21])
        fact_positions.sort()
        sample["fact_positions"] = (
            fact_positions  # positions of facts between noise sentences
        )
        # скрываем полезные факты среди шума
